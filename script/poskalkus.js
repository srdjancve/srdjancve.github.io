...
document.getElementById("rata_btn").onclick = function rata(){
    if (document.getElementById("mpc_uk").value !== "" && document.getElementById("br_rata").value !== "" && document.getElementById("uvp").value !== ""){
        mpc_uk = document.getElementById("mpc_uk").value;
        br_rata = document.getElementById("br_rata").value;
        uvp = document.getElementById("uvp").value;

        if(uvp == "0"){
            sr_vr = mpc_uk / br_rata;
        }
        else{
            sr_vr = (parseFloat(mpc_uk) + (parseFloat(mpc_uk) / 100)* parseFloat(uvp))/ parseFloat(br_rata);
        }
        
        document.getElementById("sr_vr").value = sr_vr.toFixed(2);
        document.getElementById("uk_vr").value = (sr_vr * parseFloat(br_rata)).toFixed(2);
    }
    else{
        alert("Unos ne može biti prazan!");
    }
